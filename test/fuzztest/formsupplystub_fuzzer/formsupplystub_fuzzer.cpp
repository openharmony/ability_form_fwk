...
#include "formsupplystub_fuzzer.h"
...
const std::u16string FORMMGR_INTERFACE_TOKEN = u"ohos.appexecfwk.FormSupply";
...
{
    uint32_t code = Convert2Uint32(data);
    MessageParcel datas;
    ...
    std::shared_ptr<FormSupplyStub> formsupplystub = std::make_shared<FormSupplyStubFuzzTest>();
    formsupplystub->OnRemoteRequest(code, datas, reply, option);
    return true;
}
}

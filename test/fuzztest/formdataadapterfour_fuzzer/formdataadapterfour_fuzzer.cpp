...
Want GenerateWant(FuzzedDataProvider *fdp)
{
    ...
    std::string action = GenerateSafeString(fdp, MAX_LENGTH);
...
FormItemInfo GenerateFormItemInfo(FuzzedDataProvider *fdp)
{
    ...
    std::string packageName = GenerateSafeString(fdp, MAX_LENGTH);
    formInfo.SetPackageName(packageName.empty() ? "default.package" : packageName);

    std::string providerBundleName = GenerateSafeString(fdp, MAX_LENGTH);
    formInfo.SetProviderBundleName(providerBundleName.empty() ? "default.provider.bundle" : providerBundleName);

    std::string hostBundleName = GenerateSafeString(fdp, MAX_LENGTH);
    ...
    std::string formName = GenerateSafeString(fdp, MAX_LENGTH);
    ...
    std::string scheduledUpdateTime = GenerateSafeString(fdp, MAX_STRING_LENGTH_16);
    ...
    std::string jsComponentName = GenerateSafeString(fdp, MAX_LENGTH);
    formInfo.SetJsComponentName(jsComponentName);

    std::string abilityModuleName = GenerateSafeString(fdp, MAX_LENGTH);
    formInfo.SetAbilityModuleName(abilityModuleName);

    std::string deviceId = GenerateSafeString(fdp, MAX_STRING_LENGTH_64);
    ...
    std::string description = GenerateSafeString(fdp, 256);
    ...
    std::string uiModuleName = GenerateSafeString(fdp, MAX_LENGTH);
    formInfo.SetUiModuleName(uiModuleName);

    formInfo.SetIsTemplateForm(fdp->ConsumeBool());

    std::string templateFormImperativeFwk = GenerateSafeString(fdp, 64);
...
FormRecord GenerateFormRecord(FuzzedDataProvider *fdp)
{
    ...
    // Use GenerateSafeString to ensure valid UTF-8 / ASCII for fields serialized
    // by InnerFormInfo::ToString() → nlohmann::json::dump(), which aborts on
    // invalid UTF-8 when using the default strict error handler.
    ...
    record.versionName = GenerateSafeString(fdp, MAX_STRING_LENGTH_32);
...
{
    std::string data = GenerateSafeString(fdp, MAX_DATA_SIZE);
    if (data.empty()) {
        data = "{}";
    }

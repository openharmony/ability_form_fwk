...
#include "formrefreshafteruncontrolimpl_fuzzer.h"
...
void DoSomethingInterestingWithMyAPI(const char* data, size_t size)
{
    ...
    want.SetParam("is_timer", GetU32Data(data) % 2 == 0);
    want.SetParam("timer_refresh", GetU32Data(data + 4) % 2 == 0);
    refreshData.want = want;

    FormProviderData providerData;
    refreshData.providerData = providerData;

    formRefreshAfterUncontrol.RefreshFormRequest(refreshData);
}
}

 ...
 */
 ...
 HWTEST_F(FmsFormMgrServiceTest2, FormMgrService_0138, TestSize.Level1)
 {
     ...
     bool ret = formMgrService.RegisterOverflowProxy(callerToken);
     EXPECT_TRUE(ret);
     OverflowInfo overflowInfo;
     formMgrService.RequestOverflow(formId, overflowInfo, true);
     formMgrService.RegisterChangeSceneAnimationStateProxy(callerToken);
     int32_t state = 1;
     formMgrService.ChangeSceneAnimationState(formId, state);
 ...
 HWTEST_F(FmsFormMgrServiceTest2, FormMgrService_0180, TestSize.Level1)
 {
     ...
     formMgrService.NotifyFormLocked(formId, true);
     formMgrService.UpdateFormSize(formId, width, height, borderWidth);
     bool ret = formMgrService.UnregisterOverflowProxy();
     EXPECT_TRUE(ret);
     formMgrService.UnregisterChangeSceneAnimationStateProxy();
     OverflowInfo overflowInfo;
     formMgrService.RequestOverflow(formId, overflowInfo, true);
     int32_t state = 1;
     formMgrService.ChangeSceneAnimationState(formId, state);

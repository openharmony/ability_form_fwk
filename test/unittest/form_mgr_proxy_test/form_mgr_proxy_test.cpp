...
/**
 * @tc.number: FormMgrProxyTest_RequestPublishFormCrossUser_001
 * @tc.name: Verify RequestPublishFormCrossUser with valid parameters
 * @tc.desc: test RequestPublishFormCrossUser function with valid parameters
 * @tc.type: FUNC
 */
HWTEST_F(FormMgrProxyTest, FormMgrProxyTest_RequestPublishFormCrossUser_001, TestSize.Level1) {
    GTEST_LOG_(INFO) << "FormMgrProxyTest_RequestPublishFormCrossUser_001 starts";
    Want want;
    makeWant(want, MakeWantFlag::DEFAULT);
    int32_t userId = 100;
    int64_t expectedFormId = 123456;
    EXPECT_CALL(*mockFormMgrService, RequestPublishFormCrossUser(_, userId, _))
        .Times(1)
        .WillOnce(DoAll(SetArgReferee<2>(expectedFormId), Return(ERR_OK)));
    int64_t formId;
    ErrCode result = formMgrProxy->RequestPublishFormCrossUser(want, userId, formId);
    EXPECT_EQ(result, ERR_OK);
    EXPECT_EQ(formId, expectedFormId);
    GTEST_LOG_(INFO) << "FormMgrProxyTest_RequestPublishFormCrossUser_001 ends";
}
 
/**
 * @tc.number: FormMgrProxyTest_RequestPublishFormCrossUser_002
 * @tc.name: Verify RequestPublishFormCrossUser with different userId
 * @tc.desc: test RequestPublishFormCrossUser with different userId values
 * @tc.type: FUNC
 */
HWTEST_F(FormMgrProxyTest, FormMgrProxyTest_RequestPublishFormCrossUser_002, TestSize.Level1) {
    GTEST_LOG_(INFO) << "FormMgrProxyTest_RequestPublishFormCrossUser_002 starts";
    Want want;
    makeWant(want, MakeWantFlag::DEFAULT);
    int32_t userId = 200;
    int64_t expectedFormId = 789012;
    EXPECT_CALL(*mockFormMgrService, RequestPublishFormCrossUser(_, userId, _))
        .Times(1)
        .WillOnce(DoAll(SetArgReferee<2>(expectedFormId), Return(ERR_OK)));
    int64_t formId;
    ...
    EXPECT_EQ(result, ERR_OK);
    EXPECT_EQ(formId, expectedFormId);
    GTEST_LOG_(INFO) << "FormMgrProxyTest_RequestPublishFormCrossUser_002 ends";
}
}
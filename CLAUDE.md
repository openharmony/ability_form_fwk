...
- **语言**：源码注释一律**英文**
- **版权头年份**：新建文件写当年；修改既有文件时**保留首年**并扩展为 `首年-当年`（如 `2023` → `2023-2026`）；**禁止用当年替换首年**；未实质修改文件不得单独改年份
- **简要性**：克制、简短，只写代码表达不了的 why（意图/约束/坑）；不复述代码、不写显而易见的内容，能用好命名或 HILOG 表达就不写注释；一行能说清不用多行
- **必须注释**：IPC 接口（含 TF_ASYNC/TF_SYNC 语义）、状态机转换、算法、持锁/跨线程等非显式副作用、魔法值/阈值/超时（注明取值依据）、绕过常规检查的特例
- **禁止**：注释掉的死代码（交给 git）、用注释记变更历史、零信息注释（`i++; // i 自增`）；改代码必须同步改注释

...
ErrCode FormProviderMgr::RefreshForm(const int64_t formId, const Want &want, bool isVisibleToFresh)
{
    ...
    if (currentActiveUserId != record.providerUserId) {
        ...
        HILOG_ERROR("%{public}s, not current user, just set refresh flag, currentActiveUserId:%{public}d, userId:%{public}d", __func__, currentActiveUserId, record.providerUserId);

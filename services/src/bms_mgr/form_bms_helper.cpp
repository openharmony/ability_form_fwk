...
{
    HILOG_DEBUG("call");
    uint32_t flag = (uint32_t)flags | (uint32_t)BundleFlag::GET_BUNDLE_INFO_EXCLUDE_EXT;
    sptr<IBundleMgr> iBundleMgr = GetBundleMgr();
    if (iBundleMgr == nullptr) {
        HILOG_ERROR("null iBundleMgr");
        return false;
    }
    return (IN_PROCESS_CALL(iBundleMgr->GetBundleInfo(bundleName, flag, bundleInfo, userId)));
}

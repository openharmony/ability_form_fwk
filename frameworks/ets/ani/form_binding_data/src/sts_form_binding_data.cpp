...
static ani_object CreateFormBindingData([[maybe_unused]] ani_env *env, ani_object paramObject)
{
    ...
    if ((status = env->Class_FindMethod(formBindingDataCls, "<ctor>", ":V", &objectMethod)) != ANI_OK) {
        HILOG_ERROR("Class_FindMethod failed status1 %{public}d ", static_cast<int>(status));
        return nullptr;
    }
    ani_object formBindingDataObj;
    if ((status = env->Object_New(formBindingDataCls, objectMethod, &formBindingDataObj)) != ANI_OK) {
        HILOG_ERROR("Class_FindMethod failed status2 %{public}d ", static_cast<int>(status));
        return nullptr;
    }

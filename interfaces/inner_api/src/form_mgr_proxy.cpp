...
int  FormMgrProxy::GetParcelableInfos(MessageParcel &reply, std::vector<T> &parcelableInfos)
{
    ...
    if (!reply.ReadInt32(infoSize)) {
        HILOG_ERROR("%{public}s, read infoSize failed", __func__);
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
                                     ...
                                     int32_t &numFormsDeleted)
{
    ...
    if (!reply.ReadInt32(numFormsDeleted)) {
        HILOG_ERROR("%{public}s, read numFormsDeleted failed", __func__);
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    return result;
}
...
int FormMgrProxy::AcquireFormState(const Want &want, const sptr<IRemoteObject> &callerToken, FormStateInfo &stateInfo)
{
    ...
    if (state < static_cast<int32_t>(FormState::DEFAULT) ||
        state > static_cast<int32_t>(FormState::READY)) {
        HILOG_ERROR("%{public}s, invalid state %{public}d", __func__, state);
        return ERR_APPEXECFWK_FORM_INVALID_PARAM;
    }
    stateInfo.state = static_cast<FormState>(state);
    stateInfo.want = want;
    return result;
}
...
int32_t FormMgrProxy::GetFormsCount(bool isTempFormFlag, int32_t &formCount)
{
    ...
    if (!reply.ReadInt32(formCount)) {
        HILOG_ERROR("%{public}s, read formCount failed", __func__);
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    return result;
}
...
ErrCode FormMgrProxy::GetFormInstanceById(const int64_t formId, FormInstance &formInstance)
{
    ...
    if (!data.WriteBool(false)) {
        HILOG_ERROR("%{public}s, write isUnusedInclude failed", __func__);
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    ...
    std::unique_ptr<FormProviderData> &formBindingData, int64_t &formId)
{
    ...
    if (!reply.ReadInt32(errCode)) {
        HILOG_ERROR("%{public}s, read errCode failed", __func__);
        return ERR_APPEXECFWK_PARCEL_ERROR;
    }
    if (errCode == ERR_OK) {
        if (!reply.ReadInt64(formId)) {
...
ErrCode FormMgrProxy::RegisterUpdateFormsConfigCallback(const sptr<IRemoteObject> &callerToken)
{
    ...
    MessageOption option;
    int error = SendTransactCmd(
        IFormMgr::Message::FORM_MGR_REGISTER_UPDATE_FORMS_CONFIG_CALLBACK, data, reply, option);
    if (error != ERR_OK) {
        HILOG_ERROR("send request failed, errCode: %{public}d.", error);
        return ERR_APPEXECFWK_FORM_SEND_FMS_MSG;
    }
...
ErrCode FormMgrProxy::UpdateFormsConfig(const std::vector<FormCustomConfig> &configs)
{
    ...
    MessageOption option;
    ErrCode error = SendTransactCmd(
        IFormMgr::Message::FORM_MGR_UPDATE_FORMS_CONFIG, data, reply, option);
    if (error != ERR_OK) {
        HILOG_ERROR("send request failed, errCode: %{public}d.", error);
        return ERR_APPEXECFWK_FORM_SEND_FMS_MSG;
    }

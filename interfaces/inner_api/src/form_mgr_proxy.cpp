    ...
    const sptr<IRemoteObject> &callerToken)
{
    ...
    auto error = Remote()->SendRequest(
        static_cast<uint32_t>(IFormMgr::Message::FORM_MGR_REGISTER_FORM_ADD_OBSERVER_BY_BUNDLE), data, reply, option);
    if (error != ERR_OK) {
        HILOG_ERROR("failed to SendRequest: %{public}d", error);
        return ERR_APPEXECFWK_FORM_SEND_FMS_MSG;
    }
...
int32_t FormMgrProxy::SetFormsRecyclable(const std::vector<int64_t> &formIds)
{
    ...
    MessageParcel reply;
    int32_t error = SendTransactCmd(IFormMgr::Message::FORM_MGR_SET_FORMS_RECYCLABLE, data, reply);
    if (error != ERR_OK) {
...
int32_t FormMgrProxy::RecycleForms(const std::vector<int64_t> &formIds, const Want &want)
{
    ...
    MessageParcel reply;
    int32_t error = SendTransactCmd(IFormMgr::Message::FORM_MGR_RECYCLE_FORMS, data, reply);
    if (error != ERR_OK) {
        HILOG_ERROR("failed to SendTransactCmd: %{public}d.", error);
        return error;
    }
...
int32_t FormMgrProxy::RecoverForms(const std::vector<int64_t> &formIds, const Want &want)
{
    ...
    MessageParcel reply;
    int32_t error = SendTransactCmd(IFormMgr::Message::FORM_MGR_RECOVER_FORMS, data, reply);
    if (error != ERR_OK) {
        HILOG_ERROR("failed to SendTransactCmd: %{public}d.", error);
        return error;
    }
